...
**Core Philosophy**: These are **pure CLI tools** with NO built-in LLM integration. The orchestrator (Claude Code, Cursor, etc.) calls the LLM and passes results to these tools.
...
| **2** | `llm-to-cozodb-writer` | Write temporal state | Specification | Datalog queries | Temporal flags |
| **3** | `llm-cozodb-to-context-writer` | Extract context | Optimization | Datalog queries | JSON context |
...
**IMPORTANT**: None of these tools call LLMs. The orchestrator (your AI IDE) calls the LLM and passes results to these tools.
...
### Purpose
Write temporal state changes to CozoDB based on Datalog queries (typically generated by an orchestrator after LLM reasoning).

**Key Point**: This tool does NOT call any LLM. It executes Datalog queries to update the database.
...
| Query | `-q` | `--query DATALOG` | (required) | CozoDB Datalog query to execute |
| Batch Size | `-s` | `--batch-size SIZE` | `5` | Batch size for processing |
| Dry Run | `-d` | `--dry-run` | off | Show queries without executing |
| Verbose | `-v` | `--verbose` | off | Enable verbose output |
| Quiet | `-Q` | `--quiet` | off | Suppress output except errors |
...
**Mark entity for creation** (hash-based key for new entities):
```bash
llm-to-cozodb-writer \
  --db ./parseltongue.db \
  --query "?[isgl1_key, current_ind, future_ind, future_code, future_action] := [[
    'src_handlers_rs-new_async-fn-abc12345',
    false,
    true,
...
### Purpose
Extract context from CozoDB using Datalog queries and output minimal JSON for LLM consumption.

**Key Point**: This tool does NOT call any LLM. It executes Datalog queries to read from the database.
...
|------|-------|------|---------|-------------|
| Database | `-b` | `--db PATH` | `parseltongue.db` | Database file path |
| Query | `-q` | `--query DATALOG` | (see below) | CozoDB Datalog query for extraction |
| Output | `-o` | `--output PATH` | `./contexts` | Output directory for context files |
...
llm-cozodb-to-context-writer \
  --db ./parseltongue.db \
  --output ./contexts/all_signatures.json
```
...
### Purpose
Generate `CodeDiff.json` from CozoDB for file application (ultra-minimalist, NO backups).
...
- **NO backup creation** - Trust the orchestrator
...
│ future_action: null                                         │
└─────────────────────────────────────────────────────────────┘
                    ↓ Tool 2 (Orchestrator marks for editing)
┌─────────────────────────────────────────────────────────────┐
...
**Orchestrator drives this workflow** (Claude Code, Cursor, etc.):
...
# Phase 2: Extract context for LLM analysis
llm-cozodb-to-context-writer \
...
# [ORCHESTRATOR CALLS LLM: analyzes context, identifies bug, plans changes]
# [ORCHESTRATOR GENERATES DATALOG QUERIES from LLM response]

# Phase 2 (continued): Write temporal state (orchestrator-generated queries)
llm-to-cozodb-writer \
  --db .parseltongue/parseltongue.db \
  --query "?[isgl1_key, current_ind, future_ind, future_code, future_action] := [[
    'rust:fn:buggy:src_lib_rs:42-56',
    true,
    true,
    'pub fn fixed() -> Result<()> { Ok(()) }',
    'Edit'
  ]]" \
  --verbose

# Phase 3: Extract context for changed entities
llm-cozodb-to-context-writer \
  --db .parseltongue/parseltongue.db \
  --query "?[isgl1_key, current_code, future_code, interface_signature] :=
    *CodeGraph{isgl1_key, current_code, future_code, interface_signature},
...
# [ORCHESTRATOR CALLS LLM: generates future_code for all changed entities]

# Phase 3 (continued): Write future_code (orchestrator-generated)
llm-to-cozodb-writer \
  --db .parseltongue/parseltongue.db \
  --query "..." \  # Orchestrator-generated query with future_code
  --verbose
...
# [ORCHESTRATOR READS CodeDiff.json and applies changes to files]
...
# [ORCHESTRATOR CALLS LLM with this context for analysis]
```
...
# [ORCHESTRATOR CALLS LLM to analyze coverage gaps]
```
...
**Solution**: Query with field exclusion
```bash
# Extract signatures only
llm-cozodb-to-context-writer \
  --query "?[isgl1_key, interface_signature] :=
    *CodeGraph{isgl1_key, interface_signature}" \
  --output minimal_context.json
```
...
```
PARSELTONGUE TOOLKIT - NO LLM INTEGRATION
All tools are pure CLI - orchestrator calls LLM separately

Tool 1: folder-to-cozodb-streamer --dir ./src --output-db sqlite:./db
Tool 2: llm-to-cozodb-writer --db ./db --query "DATALOG_QUERY"
Tool 3: llm-cozodb-to-context-writer --db ./db --query "DATALOG_QUERY" --output ./ctx
