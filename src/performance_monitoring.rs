...
use crate::performance_validation::{PerformanceMetrics, PerformanceValidator, WorkloadConfig};
use chrono::DateTime;
...
impl PerformanceMonitor {
    ...
    pub fn establish_baseline(&self) -> Result<PerformanceBaseline, Box<dyn std::error::Error>> {
        ...
        println!(
            "✅ Performance baseline established and saved to {}",
            self.baseline_path
        );
        Ok(baseline)
    }
    ...
    fn check_metric_regression(
        ...
        let change_percent = ((params.current_value as f64 - params.baseline_value as f64)
            / params.baseline_value as f64)
            * 100.0;
