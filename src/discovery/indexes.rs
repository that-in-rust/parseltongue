...
impl CompactEntityInfo {
    ...
    pub fn to_entity_info(&self, interner: &FileInterner) -> EntityInfo {
        let name = interner
            .get_name(self.name_id)
            .unwrap_or("unknown")
            .to_string();
        let file_path = interner
...
impl DiscoveryIndexes {
    ...
    pub fn filter_entities_by_type(
        &self,
        entity_type: EntityType,
    ...
    pub fn filter_entities_by_type_vectorized(
        &self,
        entity_type: EntityType,
    ) -> impl Iterator<Item = &CompactEntityInfo> {
    ...
    pub fn filter_entities_by_types_batch<'a>(
        ...
        self.all_entities.iter().filter_map(move |entity| {
    ...
    pub fn filter_entities_with_predicates<F>(
        &self,
        predicate: F,
    ) -> impl Iterator<Item = &CompactEntityInfo>
    where
    ...
    pub fn find_entities_limited<F>(
        &self,
        predicate: F,
        limit: usize,
    ) -> impl Iterator<Item = &CompactEntityInfo>
    ...
    pub fn filter_entities_by_file(
        &self,
        file_id: FileId,
    ) -> impl Iterator<Item = &CompactEntityInfo> {
        self.all_entities
            .iter()
            .filter(move |entity| entity.file_id == file_id)
    }
    ...
    pub fn filter_entities_by_name_prefix<'a>(
        ...
        self.all_entities.iter().filter(move |entity| {
    ...
    pub fn memory_stats(&self) -> MemoryStats {
        ...
        let file_index_memory = self.file_index.len()
            * (std::mem::size_of::<FileId>() + std::mem::size_of::<Vec<usize>>())
            + self
                .file_index
                .values()
                .map(|v| v.len() * std::mem::size_of::<usize>())
                .sum::<usize>();
        let type_index_memory = self.type_index.len()
            * (std::mem::size_of::<EntityType>() + std::mem::size_of::<Vec<usize>>())
            + self
                .type_index
                .values()
                .map(|v| v.len() * std::mem::size_of::<usize>())
        ...
        MemoryStats {
            ...
            total_memory: entity_memory
                + file_index_memory
                + type_index_memory
                + self.interner.memory_usage_bytes(),
        }
    }
}
...
mod memory_optimization_tests {
    ...
    fn test_zero_allocation_entity_filtering_performance_contract() {
        ...
        assert!(
            zero_alloc_time < Duration::from_millis(1),
            "Zero-allocation filtering took {:?}, expected <1ms",
            zero_alloc_time
        );
    }
    ...
    fn test_batch_entity_filtering_with_multiple_types() {
        ...
        assert!(
            ...
            "Batch filtering took {:?}, expected <5ms",
            elapsed
        );
    ...
    fn test_memory_efficient_pagination() {
        ...
        assert!(
            ...
            "Pagination took {:?}, expected <500μs",
            elapsed
        );
    ...
    fn test_name_prefix_filtering_performance() {
        ...
        assert!(
            ...
            "Name prefix filtering took {:?}, expected <2ms",
            elapsed
        );

        assert!(
            !test_matches.is_empty(),
            "Should find test_ prefixed entities"
        );
        assert!(
            !util_matches.is_empty(),
            "Should find util_ prefixed entities"
        );
        ...
        for entity in &test_matches {
            ...
            assert!(
                ...
                "Entity name should start with test_"
            );
        }
    }
...
mod tests {
    ...
    fn test_compact_entity_info_memory_layout() {
        ...
        assert_eq!(
            std::mem::size_of::<CompactEntityInfo>(),
            24,
    ...
    fn test_rebuild_from_entities_single() {
        let mut indexes = DiscoveryIndexes::new();
        let entities = vec![EntityInfo::new(
    ...
    fn test_memory_stats() {
        ...
        assert_eq!(
            stats.entity_memory,
            2 * std::mem::size_of::<CompactEntityInfo>()
        );
    ...
    fn test_performance_contract_large_codebase() {
        ...
        assert!(
            ...
            "Index rebuild took {:?}, expected <5s",
            elapsed
        );
    ...
    fn test_performance_contract_very_large_codebase() {
        ...
        assert!(
            elapsed < Duration::from_secs(5),
            "Index rebuild took {:?}, expected <5s for 100k entities",
            elapsed
        );
        ...
        assert!(
            ...
            "Memory usage too high: {} bytes per entity",
            bytes_per_entity
        );
        ...
        for entity_type in [
            ...
            assert!(
                entities_of_type.len() > 10_000,
                "Expected >10k entities of type {:?}, got {}",
                entity_type,
    ...
    fn test_to_entity_info_conversion() {
        ...
        indexes
            .rebuild_from_entities(vec![original.clone()])
            .unwrap();
    ...
    fn test_complete_discovery_workflow() {
        ...
        assert!(
            rebuild_time < Duration::from_millis(100),
            "Rebuild should be fast for small datasets"
        );
