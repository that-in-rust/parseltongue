...
impl NodeConverter {
    ...
    pub fn from_node_data(node_data: &NodeData, interner: &mut FileInterner) -> EnhancedIsgNode {
...
mod tests {
    ...
    fn test_format_location() {
        ...
        assert_eq!(
            node_complete.format_location(&interner),
            "src/main.rs:42:10"
        );
    ...
    fn test_o1_file_location_access_performance_large_dataset() {
        ...
        assert!(
            avg_time_ns < 1000,
            "File location access too slow: {} ns > 1000 ns",
            avg_time_ns
        );
        ...
        assert!(
            avg_time_ns < 1000,
            "File path access too slow: {} ns > 1000 ns",
            avg_time_ns
        );
    }
    ...
    fn test_o1_scalability_independence() {
        ...
        for &time in &access_times[1..] {
            ...
            assert!(
                ratio < 3.0,
                "Access time increased too much with dataset size: {}x",
                ratio
            );
        }
    }
    ...
    fn test_conversion_performance() {
        ...
        // Should be fast - under 10ms for 1000 nodes
        assert!(
            back_conversion_time.as_millis() < 10,
            "Batch back-conversion too slow: {:?}",
            back_conversion_time
        );
    ...
    fn test_memory_efficiency_with_interning() {
        ...
        assert!(
            memory_usage.total_bytes() < 10000,
    ...
    fn test_entity_info_conversion_performance() {
        ...
        assert!(
            elapsed.as_millis() < 10,
            "EntityInfo conversion too slow: {:?}",
            elapsed
        );
