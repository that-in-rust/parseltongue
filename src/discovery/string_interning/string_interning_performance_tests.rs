...
mod performance_tests {
    ...
    fn test_string_interning_efficiency() {
        ...
        assert!(
            ...
            "Memory usage per entry too high: {}",
            bytes_per_entry
        );
    ...
    fn test_memory_usage_scaling() {
        ...
        assert!(
            total_mb < 1.0,
            "Memory usage too high: {:.2} MB for 1000 entries",
            total_mb
        );
    ...
    fn test_interning_performance() {
        ...
        assert!(
            ...
            "Interning performance too slow: {:.0} ops/sec",
            ops_per_second
        );
        ...
        assert!(
            lookup_ops_per_second > 50000.0,
            "Lookup performance too slow: {:.0} ops/sec",
    ...
    fn test_memory_efficiency_comparison() {
        ...
        assert!(
            savings_ratio > 5.0,
            "String interning not efficient enough. Savings ratio: {:.2}",
            savings_ratio
        );
    ...
    fn test_realistic_codebase_simulation() {
        ...
        assert!(
            ...
            "Realistic simulation too slow: {}ms",
            elapsed.as_millis()
        );

        // Memory usage should be reasonable
        assert!(
            usage.total_bytes() < 10000,
        ...
        println!(
            "  Efficiency: {:.2} entities/byte",
            total_entities as f64 / usage.total_bytes() as f64
        );
    }
}
