...
impl DiscoveryError {
    ...
    pub fn is_data_integrity_issue(&self) -> bool {
        matches!(self, Self::IndexCorruption { .. } | Self::Internal { .. })
    }
    ...
    pub fn category(&self) -> ErrorCategory {
        match self {
            ...
            Self::QueryTimeout { .. } | Self::PerformanceViolation { .. } => {
...
impl PerformanceMonitor {
    ...
    pub fn check_memory_usage(
        ...
        if increase_percent > self.memory_limit_increase_percent {
            let limit_mb = baseline_mb
                + ((baseline_mb as f64 * self.memory_limit_increase_percent / 100.0) as usize);
...
pub fn add_discovery_context<T>(result: DiscoveryResult<T>, context: &str) -> anyhow::Result<T> {
    ...
    pub fn format_error(&self, error: &DiscoveryError) -> String {
        ...
        match error.category() {
            ErrorCategory::NotFound => {
                ...
                message
                    .push_str("  • Use 'parseltongue list-entities' to see available entities\n");
...
pub fn create_cli_error(error: DiscoveryError, operation: &str) -> anyhow::Error {
    let context = match &error {
        DiscoveryError::EntityNotFound { name } => ErrorContext::new(operation)
            .with_suggestion(
                "Try 'parseltongue list-entities --type functions' to see available functions"
                    .to_string(),
            )
            .with_suggestion(format!(
                "Search for similar names: 'parseltongue list-entities | grep {}'",
                name
            ))
            .with_related_command("parseltongue list-entities")
            .with_related_command("parseltongue entities-in-file <file>"),
        DiscoveryError::FileNotFound { path: _ } => ErrorContext::new(operation)
            .with_suggestion("Check if the file path is correct")
            .with_suggestion("Use relative paths from the project root")
            .with_related_command(
                "parseltongue list-entities --json | jq '.[] | .file_path' | sort | uniq",
            ),
        DiscoveryError::InvalidQuery { .. } => ErrorContext::new(operation)
        ...
        DiscoveryError::QueryTimeout { .. } => ErrorContext::new(operation)
        ...
        DiscoveryError::PerformanceViolation { .. } => ErrorContext::new(operation)
...
mod tests {
    ...
    fn test_error_creation() {
        ...
        assert!(
            matches!(error, DiscoveryError::EntityNotFound { name } if name == "test_function")
        );
        ...
        assert!(
            matches!(error, DiscoveryError::InvalidQuery { reason } if reason == "empty query")
        );

        let error = DiscoveryError::query_timeout("list_all", Duration::from_secs(5));
        assert!(
            matches!(error, DiscoveryError::QueryTimeout { query, limit }
    ...
    fn test_all_error_variants_creation() {
        ...
        let _ = DiscoveryError::performance_violation(
            "op",
    ...
    fn test_error_categories() {
        assert_eq!(
            DiscoveryError::entity_not_found("test").category(),
            ErrorCategory::NotFound
        );
        assert_eq!(
            DiscoveryError::file_not_found("test").category(),
            ErrorCategory::NotFound
        );
        assert_eq!(
            DiscoveryError::invalid_query("test").category(),
            ErrorCategory::InvalidInput
        );
        assert_eq!(
            DiscoveryError::query_timeout("test", Duration::from_secs(1)).category(),
            ErrorCategory::Performance
        );
        assert_eq!(
            DiscoveryError::performance_violation(
                "test",
                Duration::from_millis(200),
                Duration::from_millis(100)
            )
        ...
        assert_eq!(
            DiscoveryError::memory_limit_exceeded(100, 50).category(),
            ErrorCategory::Resource
        );
        assert_eq!(
            DiscoveryError::index_corruption("test").category(),
            ErrorCategory::DataIntegrity
        );
        assert_eq!(
            DiscoveryError::concurrency_error("test").category(),
            ErrorCategory::Concurrency
        );
        assert_eq!(
            DiscoveryError::serialization_error("test").category(),
            ErrorCategory::Serialization
        );
        assert_eq!(
            DiscoveryError::internal("test").category(),
            ErrorCategory::Internal
        );
    }
    ...
    fn test_performance_monitor_creation() {
        ...
        assert_eq!(
            custom_monitor.discovery_time_limit,
            Duration::from_millis(200)
        );
        assert_eq!(
            custom_monitor.existing_query_limit,
            Duration::from_micros(100)
        );
    ...
    fn test_performance_monitor_discovery_performance_check() {
        ...
        let result =
            monitor.check_discovery_performance("list_entities", Duration::from_millis(50));
        assert!(result.is_ok());

        // Should fail for slow operations
        let result =
            monitor.check_discovery_performance("list_entities", Duration::from_millis(150));
    ...
    fn test_performance_monitor_existing_query_performance_check() {
        ...
        let result =
            monitor.check_existing_query_performance("blast_radius", Duration::from_micros(25));
        assert!(result.is_ok());

        // Should fail for slow queries
        let result =
            monitor.check_existing_query_performance("blast_radius", Duration::from_micros(100));
        assert!(result.is_err());
    ...
    fn test_performance_issue_classification() {
        ...
        for error in performance_errors {
            assert!(
                error.is_performance_issue(),
                "Error should be a performance issue: {:?}",
                error
            );
        }
        ...
        for error in non_performance_errors {
            assert!(
                !error.is_performance_issue(),
                "Error should not be a performance issue: {:?}",
                error
            );
        }
    }
    ...
    fn test_data_integrity_issue_classification() {
        ...
        for error in integrity_errors {
            assert!(
                error.is_data_integrity_issue(),
                "Error should be a data integrity issue: {:?}",
                error
            );
        }
        ...
        for error in non_integrity_errors {
            assert!(
                !error.is_data_integrity_issue(),
                "Error should not be a data integrity issue: {:?}",
                error
            );
        }
    }
