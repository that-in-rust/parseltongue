...
impl FileInterner {
    ...
    pub fn memory_usage(&self) -> MemoryUsage {
        let path_map_size = self.path_to_id.len()
            * (std::mem::size_of::<Arc<str>>() + std::mem::size_of::<FileId>());
        let id_map_size = self.id_to_path.len()
            * (std::mem::size_of::<FileId>() + std::mem::size_of::<Arc<str>>());

        // Estimate string storage (approximate)
        let string_storage: usize = self.id_to_path.values().map(|arc| arc.len()).sum();
    ...
    pub fn compact(&mut self) {
        ...
        let mut new_path_to_id =
            FxHashMap::with_capacity_and_hasher(current_len, Default::default());
        let mut new_id_to_path =
            FxHashMap::with_capacity_and_hasher(current_len, Default::default());
...
impl TrigramIndex {
    ...
    pub fn build_from_interner(&mut self, interner: &FileInterner) {
        ...
        for (id, path_arc) in &interner.id_to_path {
            ...
            for trigram in trigrams {
                self.trigram_to_ids.entry(trigram).or_default().push(*id);
    ...
    pub fn compact_with_compression(&mut self) {
        ...
        self.trigram_to_ids
            .retain(|_trigram, ids| ids.len() >= min_matches);
    }
    ...
    pub fn memory_usage(&self) -> usize {
        let map_overhead = self.trigram_to_ids.len()
            * (std::mem::size_of::<[u8; 3]>() + std::mem::size_of::<Vec<FileId>>());
        let vector_storage: usize = self
            .trigram_to_ids
            .values()
