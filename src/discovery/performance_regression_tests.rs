...
use crate::isg::{NodeData, NodeKind, OptimizedISG, SigHash};
use std::sync::Arc;
...
impl PerformanceRegressionTester {
    ...
    pub async fn test_discovery_performance_contract(
        &self,
        max_results: usize,
        ...
        self.metrics
            .validate_discovery_performance("list_all_entities", elapsed)?;

        Ok(elapsed)
    }

    /// Test existing query performance contract (<50μs)
    pub async fn test_existing_query_performance_contract(
        &self,
        entity_name: &str,
    ) -> Result<Duration, MetricsError> {
        let start = Instant::now();
        ...
        // Validate against contract
        self.metrics
            .validate_existing_query_performance("where_defined", elapsed)?;

        Ok(elapsed)
    }

    /// Test file-based discovery performance
    pub async fn test_file_discovery_performance(
        &self,
        file_path: &str,
    ) -> Result<Duration, MetricsError> {
        let start = Instant::now();
        ...
        self.metrics
            .validate_discovery_performance("entities_in_file", elapsed)?;
    ...
    pub async fn run_comprehensive_test_suite(&mut self) -> PerformanceTestResults {
        ...
        match self
            .test_existing_query_performance_contract("test_entity_0")
            .await
        {
...
mod tests {
    ...
    async fn test_file_discovery_performance() {
        ...
        let result = tester
            .test_file_discovery_performance("src/test_0.rs")
            .await;
        assert!(result.is_ok());
    ...
    async fn test_performance_contract_violation_detection() {
        ...
        let result = tester
            .metrics
            .validate_discovery_performance("slow_operation", slow_duration);
    ...
    async fn test_performance_regression_detection() {
        ...
        tester
            .metrics
            .record_discovery_operation("test_op", Duration::from_millis(50));
        tester
            .metrics
            .record_discovery_operation("test_op", Duration::from_millis(60));
        tester
            .metrics
            .record_discovery_operation("test_op", Duration::from_millis(55));

        // Test with significantly slower performance (100ms vs ~55ms baseline)
        let regression_check = tester
            .metrics
            .detect_performance_regression("test_op", Duration::from_millis(100));
