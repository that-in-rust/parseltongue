...
impl DiscoveryMetrics {
    ...
    pub fn set_baseline_memory(&mut self, memory_mb: usize) {
        self.baseline_memory_mb
            .store(memory_mb as u64, Ordering::Relaxed);
    }
    ...
    pub fn detect_performance_regression(
        ...
        let degradation_percent = if baseline_mean.as_nanos() > 0 {
            ((current_duration.as_nanos() as f64 - baseline_mean.as_nanos() as f64)
                / baseline_mean.as_nanos() as f64)
                * 100.0
        } else {
            0.0
        };
...
mod tests {
    ...
    fn test_discovery_performance_contract_validation() {
        ...
        let fast_result =
            metrics.validate_discovery_performance("list_entities", Duration::from_millis(50));
        ...
        let slow_result =
            metrics.validate_discovery_performance("list_entities", Duration::from_millis(150));
        assert!(slow_result.is_err());

        if let Err(MetricsError::ContractViolation {
            operation,
            actual,
            limit,
        }) = slow_result
    ...
    fn test_existing_query_performance_contract_validation() {
        ...
        let fast_result =
            metrics.validate_existing_query_performance("blast_radius", Duration::from_micros(25));
        assert!(fast_result.is_ok());
        let validation = fast_result.unwrap();
        assert!(validation.passed);
        ...
        let slow_result =
            metrics.validate_existing_query_performance("blast_radius", Duration::from_micros(100));
        assert!(slow_result.is_err());
    }
    ...
    fn test_memory_usage_monitoring() {
        ...
        if let Err(MetricsError::MemoryLimitExceeded {
            current_mb,
            limit_mb,
        }) = excessive_result
        {
        ...
        for i in 0..10 {
            ...
            handles.push(thread::spawn(move || {
                for j in 0..100 {
                    ...
                    metrics_clone
                        .discovery_duration
                        .record(Duration::from_millis(i * 10 + j));
                }
            }));
        }
...
mod micro_benchmark_tests {
    use super::*;
    use crate::discovery::{
        indexes::{CompactEntityInfo, DiscoveryIndexes},
    ...
    fn test_micro_benchmark_entity_filtering_performance() {
        ...
        assert!(
            ...
            "Entity filtering took {:?} per iteration, expected <50μs",
            per_iteration
        );
    }
    ...
    fn test_micro_benchmark_string_interning_performance() {
        ...
        assert!(
            per_intern < Duration::from_micros(1),
            "String interning took {:?} per operation, expected <1μs",
            per_intern
        );
        ...
        assert!(
            bytes_per_entry < 200,
            "String interning uses {} bytes per entry, expected <200",
            bytes_per_entry
        );
    }
    ...
    fn test_micro_benchmark_memory_stats_calculation() {
        ...
        assert!(
            ...
            "Memory stats calculation took {:?}, expected <10μs",
            per_calculation
        );
    }
    ...
    fn test_micro_benchmark_compact_entity_memory_layout() {
        ...
        assert_eq!(
            size, 24,
            "CompactEntityInfo must be exactly 24 bytes, got {}",
            size
        );
        assert_eq!(
            align, 8,
            "CompactEntityInfo must be 8-byte aligned, got {}",
            align
        );
        ...
        assert!(
            entities_per_cache_line >= 2,
            "Should fit at least 2 entities per cache line, got {}",
            entities_per_cache_line
        );
    }
    ...
    fn test_micro_benchmark_index_rebuild_scalability() {
        ...
        if rebuild_times.len() >= 2 {
            ...
            let time_ratio =
                rebuild_times[1].as_nanos() as f64 / rebuild_times[0].as_nanos() as f64;
