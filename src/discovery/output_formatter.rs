...
use crate::discovery::{
    ComplexityLevel, ConfidenceLevel, DebugResult, FeaturePlanResult, OnboardingResult, Priority,
    RefactorResult,
};
...
impl OutputFormatter for HumanFormatter {
    fn format_onboarding(&self, result: &OnboardingResult) -> Result<String, FormattingError> {
        ...
        output.push_str(&format!(
            "{}PARSELTONGUE ONBOARDING PROTOCOL ACTIVATED\n",
            emoji_prefix
        ));
        output.push_str("═══════════════════════════════════════════════\n\n");
        ...
        if !result.key_contexts.is_empty() {
            ...
            output.push_str(&format!(
                "{}Sanctum Sanctorum - Key Knowledge:\n",
                context_emoji
            ));
        ...
        if !result.next_steps.is_empty() {
            ...
            output.push_str(&format!(
                "{}Mission Briefing - Next Objectives:\n",
                steps_emoji
            ));
        ...
        if self.include_timing {
            ...
            output.push_str(&format!(
                "{}Speed Analysis: {:.2}s {} (target: <15 minutes)\n",
                timing_emoji,
    ...
    fn format_feature_plan(&self, result: &FeaturePlanResult) -> Result<String, FormattingError> {
        ...
        output.push_str(&format!(
            "{}STARK INDUSTRIES FEATURE DEVELOPMENT PROTOCOL\n",
            emoji_prefix
        ));
        output.push_str("═══════════════════════════════════════════════════\n\n");
        ...
        output.push_str(&format!(
            "  💥 Direct Impact: {} entities\n",
            result.impact_analysis.direct_impact.len()
        ));
        output.push_str(&format!(
            "  🌊 Ripple Effect: {} entities\n",
        ...
        if self.include_timing {
            ...
            output.push_str(&format!(
                "{}Stark Tech Efficiency: {:.2}s {} (target: <5 minutes)\n",
                timing_emoji,
                result.execution_time.as_secs_f64(),
                time_status
            ));
    ...
    fn format_debug(&self, result: &DebugResult) -> Result<String, FormattingError> {
        ...
        output.push_str(&format!(
            "{}SPIDER-SENSE DEBUG PROTOCOL ACTIVATED\n",
            emoji_prefix
        ));
        output.push_str("═══════════════════════════════════════════\n\n");
        ...
        if self.include_timing {
            ...
            output.push_str(&format!(
                "{}Web-Swing Speed: {:.2}s {} (target: <2 minutes)\n",
                timing_emoji,
                result.execution_time.as_secs_f64(),
                time_status
            ));
    ...
    fn format_refactor(&self, result: &RefactorResult) -> Result<String, FormattingError> {
        ...
        output.push_str(&format!(
            "{}HULK SMASH... CAREFULLY! REFACTOR PROTOCOL\n",
            emoji_prefix
        ));
        output.push_str("═══════════════════════════════════════════════\n\n");
        output.push_str(&format!(
        ...
        if !result.change_checklist.is_empty() {
            ...
            output.push_str(&format!(
                "{}Hulk's Careful Transformation Steps:\n",
                checklist_emoji
            ));
        ...
        let reviewer_emoji = if self.use_emojis {
            "🦸‍♂️ "
        } else {
            ""
        };
        output.push_str(&format!(
            "{}Avengers Team Review Protocol:\n",
            reviewer_emoji
        ));
        ...
        if self.include_timing {
            ...
            output.push_str(&format!(
                "{}Transformation Speed: {:.2}s {} (target: <3 minutes)\n",
                timing_emoji,
                result.execution_time.as_secs_f64(),
                time_status
            ));
...
impl OutputFormatter for CiFormatter {
        ...
        match self.platform {
            CiPlatform::GitHub => {
                ...
                if self.export_variables {
                    ...
                    output.push_str(&format!(
                        "echo \"TOTAL_FILES={}\" >> $GITHUB_ENV\n",
                        result.overview.total_files
                    ));
                    output.push_str(&format!(
                        "echo \"TOTAL_ENTITIES={}\" >> $GITHUB_ENV\n",
                        result.overview.total_entities
                    ));
                    ...
                    if !result.overview.architecture_patterns.is_empty() {
                        ...
                        output.push_str(&format!(
                            "echo \"ARCHITECTURE_PATTERNS={}\" >> $GITHUB_ENV\n",
                            patterns
                        ));
                    }
    ...
    fn format_feature_plan(&self, result: &FeaturePlanResult) -> Result<String, FormattingError> {
        ...
        let complexity_str =
            format!("{:?}", result.impact_analysis.complexity_estimate).to_uppercase();

        match self.platform {
            CiPlatform::GitHub => {
                // Risk-based notifications
                match result.impact_analysis.risk_level {
                    crate::discovery::WorkflowRiskLevel::High
                    | crate::discovery::WorkflowRiskLevel::Critical => {
                        output.push_str(&format!(
                            "::error title=High Risk Feature::Feature {} has high risk level\n",
                            result.target_entity
                        ));
                    }
                    ...
                    crate::discovery::WorkflowRiskLevel::Low => {
                        output.push_str(&format!(
                            "::notice title=Low Risk Feature::Feature {} is low risk\n",
                            result.target_entity
                        ));
                    }
                }
                ...
                if self.export_variables {
                    output.push_str(&format!(
                        "echo \"RISK_LEVEL={}\" >> $GITHUB_ENV\n",
                        risk_level_str
                    ));
                    output.push_str(&format!(
                        "echo \"COMPLEXITY={}\" >> $GITHUB_ENV\n",
                        complexity_str
                    ));
                    output.push_str(&format!(
                        "echo \"TARGET_ENTITY={}\" >> $GITHUB_ENV\n",
                        result.target_entity
                    ));
                    output.push_str(&format!(
                        "echo \"REQUIRED_TESTS={}\" >> $GITHUB_ENV\n",
                    ...
                    let approval_required = match result.impact_analysis.risk_level {
                        crate::discovery::WorkflowRiskLevel::High
                        | crate::discovery::WorkflowRiskLevel::Critical => "true",
                        _ => "false",
                    };
                    output.push_str(&format!(
                        "echo \"APPROVAL_REQUIRED={}\" >> $GITHUB_ENV\n",
                        approval_required
                    ));
                }
            ...
            CiPlatform::GitLab => {
                output.push_str(&format!(
                    "echo \"🎯 Feature planning for {} complete\"\n",
                    result.target_entity
                ));
    ...
    fn format_debug(&self, result: &DebugResult) -> Result<String, FormattingError> {
        ...
        match self.platform {
            CiPlatform::GitHub => {
                output.push_str(&format!(
                    "::notice title=Debug Analysis::Analysis complete for {}\n",
                    result.target_entity
                ));

                if self.export_variables {
                    output.push_str(&format!(
                        "echo \"DEBUG_TARGET={}\" >> $GITHUB_ENV\n",
                        result.target_entity
                    ));
                    output.push_str(&format!(
                        "echo \"CALLER_COUNT={}\" >> $GITHUB_ENV\n",
                        result.caller_traces.len()
                    ));
                    output.push_str(&format!(
                        "echo \"USAGE_SITES={}\" >> $GITHUB_ENV\n",
                        result.usage_sites.len()
                    ));
                    output.push_str(&format!(
                        "echo \"FILES_TO_CHANGE={}\" >> $GITHUB_ENV\n",
            ...
            CiPlatform::GitLab => {
                output.push_str(&format!(
                    "echo \"🐛 Debug analysis for {} complete\"\n",
                    result.target_entity
                ));
        ...
        match self.platform {
            CiPlatform::GitHub => {
                ...
                match result.risk_assessment.overall_risk {
                    ...
                    crate::discovery::WorkflowRiskLevel::Low => {
                        output.push_str(&format!(
                            "::notice title=Low Risk Refactor::Refactoring {} is low risk\n",
                            result.target_entity
                        ));
                    }
                }
                ...
                if self.export_variables {
                    ...
                    // Safety checks count
                    let incomplete_checks = result
                        .change_checklist
                        .iter()
                        .filter(|item| !item.completed)
                        .count();
                    output.push_str(&format!(
                        "echo \"SAFETY_CHECKS={}\" >> $GITHUB_ENV\n",
                        incomplete_checks
                    ));
                }
            ...
            CiPlatform::GitLab => {
                output.push_str(&format!(
                    "echo \"🔧 Refactor safety check for {} complete\"\n",
                    result.target_entity
                ));
...
mod tests {
    use super::*;
    use crate::discovery::{
        ChangeScope, ChecklistItem, CodebaseOverview, ComplexityLevel, ConfidenceLevel,
        DebugResult, EntryPoint, FeaturePlanResult, FileLocation, ImpactAnalysis, KeyContext,
        ModuleInfo, OnboardingResult, Priority, RefactorResult, ReviewerGuidance, RiskAssessment,
        RiskFactor, ScopeGuidance, TestRecommendation,
    };
    ...
    fn test_formatter_performance_contract() {
        ...
        assert!(
            ...
            "Formatter creation took {:?}, expected <10ms",
            elapsed
        );
    }
    ...
    fn test_formatter_consistency_across_formats() {
        ...
        assert!(human_formatter
            .format_onboarding(&onboarding_result)
            .is_ok());
        ...
        let human_output = human_formatter
            .format_onboarding(&onboarding_result)
            .unwrap();
