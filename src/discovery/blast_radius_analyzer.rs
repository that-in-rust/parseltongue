...
use crate::isg::{EdgeKind, NodeData, OptimizedISG, SigHash};
...
impl BlastRadiusAnalysis {
    ...
    pub fn format_summary(&self) -> String {
        ...
        summary.push_str(&format!(
            "📈 Total Impact: {} entities\n",
            self.total_impact_count
        ));
        summary.push_str(&format!(
            "🏭 Production Impact: {} entities\n",
            self.production_impacts.len()
        ));
        summary.push_str(&format!(
            "🧪 Test Impact: {} entities\n\n",
...
impl BlastRadiusAnalyzer {
    ...
    pub fn analyze_blast_radius(&self, entity_name: &str) -> Result<BlastRadiusAnalysis> {
        ...
        let impacts_with_relationships =
            self.detect_relationship_types(target_hash, &blast_radius)?;
    ...
    fn group_impacts_by_relationship(
        ...
        for (node, edge_kind) in impacts {
            ...
            groups.entry(edge_kind).or_default().push(impacted_entity);
        }
...
mod tests {
    ...
    fn create_test_isg() -> OptimizedISG {
        ...
        isg.upsert_edge(main_fn.hash, user_service.hash, EdgeKind::Calls)
            .unwrap();

        // UserService implements Database trait
        isg.upsert_edge(user_service.hash, database_trait.hash, EdgeKind::Implements)
            .unwrap();

        // Test uses UserService
        isg.upsert_edge(test_user_service.hash, user_service.hash, EdgeKind::Uses)
            .unwrap();
    ...
    fn test_analyze_blast_radius_returns_readable_output() {
        ...
        for group in &analysis.impact_groups {
            for entity in &group.entities {
                assert!(
                    !entity.name.contains("SigHash"),
                    "Entity name should not contain hash: {}",
                    entity.name
                );
                assert!(
                    !entity.file_path.is_empty(),
    ...
    fn test_impact_group_structure_by_relationship_type() {
        ...
        let relationship_types: HashSet<EdgeKind> = analysis
            .impact_groups
            .iter()
        ...
        for group in &analysis.impact_groups {
            assert!(
                ...
                "Each group should have entities"
            );
        }
    }
    ...
    fn test_separation_of_test_files_from_production() {
        ...
        assert_eq!(
            total_impacts, analysis.total_impact_count,
            "All impacts should be categorized"
        );

        // Verify test files are properly identified
        for test_entity in &analysis.test_impacts {
            assert!(
                test_entity.is_test_file,
                "Test entity should be marked as test file"
            );
            assert!(
                test_entity.file_path.contains("test")
                    || test_entity.file_path.contains("spec")
                    || test_entity.file_path.ends_with("_test.rs"),
                "Test file path should indicate it's a test: {}",
                test_entity.file_path
            );
        }
        ...
        for prod_entity in &analysis.production_impacts {
            assert!(
                !prod_entity.is_test_file,
                "Production entity should not be marked as test file"
            );
        }
    }
    ...
    fn test_is_test_file_detection() {
        ...
        assert!(BlastRadiusAnalyzer::is_test_file(
            "tests/integration/api_test.rs"
        ));
        assert!(BlastRadiusAnalyzer::is_test_file(
            "benches/benchmark_test.rs"
        ));
    ...
    fn test_100_percent_readable_output_no_hash_values() {
        ...
        assert!(
            !json_output.contains("SigHash"),
            "Output should not contain SigHash"
        );
        assert!(
            !json_output.contains("0x"),
            "Output should not contain hex values"
        );

        // Should contain human-readable information
        assert!(
            json_output.contains("UserService"),
            "Should contain target entity name"
        );
        assert!(
            json_output.contains("src/"),
            "Should contain readable file paths"
        );
    }
    ...
    fn test_performance_contract_under_1ms() {
        ...
        assert!(
            elapsed.as_millis() < 1,
            "Analysis should complete in <1ms, took {:?}",
            elapsed
        );
    }
    ...
    fn test_production_impact_percentage_calculation() {
        ...
        if analysis.total_impact_count > 0 {
            let expected = (analysis.production_impacts.len() as f64
                / analysis.total_impact_count as f64)
                * 100.0;
