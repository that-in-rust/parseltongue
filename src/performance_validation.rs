...
use crate::isg::{EdgeKind, NodeData, NodeKind, OptimizedISG, SigHash};
use std::path::Path;
use std::sync::Arc;
...
impl RealisticDataGenerator {
    ...
    pub fn generate_code_dump(
        ...
        for file_idx in 0..config.file_count {
            ...
            let generated_lines = structs_per_file * 6
                + traits_per_file * 5
...
impl PerformanceValidator {
    ...
    fn validate_file_operations(&self, config: &WorkloadConfig) -> FileOperationMetrics {
        ...
        self.generator
            .generate_code_dump(config, &dump_path)
            .unwrap();
    ...
    fn validate_memory_usage(&self, isg: &OptimizedISG, _config: &WorkloadConfig) -> MemoryMetrics {
        ...
        let estimated_memory_bytes = (node_count * (node_size_bytes + index_overhead_bytes))
        ...
        let memory_per_node_bytes = if node_count > 0 {
            estimated_memory_bytes / node_count
        } else {
            0
        };
        let memory_per_edge_bytes = if edge_count > 0 {
            (edge_count * edge_size_bytes) / edge_count
        } else {
            0
        };
...
mod tests {
    ...
    fn test_large_workload_performance_contracts() {
        ...
        assert!(
            metrics.file_operations.update_time_ms < 12,
        ...
        assert!(
            metrics.file_operations.ingestion_time_s < 10.0,
        ...
        assert!(
            metrics.memory_usage.total_memory_mb < 25,
        ...
        assert!(
            metrics.memory_usage.memory_per_node_bytes < 500,
        ...
        println!(
            "   📊 Node operations: {}μs upsert, {}μs get, {}μs lookup",
            metrics.node_operations.upsert_time_us,
            metrics.node_operations.get_time_us,
            metrics.node_operations.lookup_time_us
        );
        ...
        println!(
            "   📊 Memory usage: {}MB total ({} bytes/node, {} bytes/edge)",
            metrics.memory_usage.total_memory_mb,
            metrics.memory_usage.memory_per_node_bytes,
            metrics.memory_usage.memory_per_edge_bytes
        );
        println!(
            "   📊 Platform: {} (hash consistency: {})",
    ...
    fn test_extra_large_workload_stress_test() {
        ...
        println!(
            "🔥 Stress testing with extreme workload: {} LOC",
            config.lines_of_code
        );
        println!(
            "   Target: {} nodes, {} edges, {} files",
        ...
        assert!(
            metrics.node_operations.upsert_time_us < 100,
            "❌ Node upsert took {}μs (>100μs stress test) - System cannot handle extreme load",
            metrics.node_operations.upsert_time_us
        );
        assert!(
            metrics.query_operations.blast_radius_time_us < 2000,
        ...
        assert!(
            metrics.memory_usage.total_memory_mb < 50,
        ...
        println!(
            "   📊 Extreme load handled: {} nodes, {} edges, {} LOC",
            config.node_count, config.edge_count, config.lines_of_code
        );
        println!(
            "   📊 Performance under stress: {}μs upsert, {}μs blast-radius, {}ms update",
            metrics.node_operations.upsert_time_us,
            metrics.query_operations.blast_radius_time_us,
            metrics.file_operations.update_time_ms
        );
        println!(
            "   📊 Memory efficiency: {}MB total ({} bytes/node)",
            metrics.memory_usage.total_memory_mb, metrics.memory_usage.memory_per_node_bytes
        );
    }
    ...
    fn test_medium_workload_baseline() {
        ...
        assert!(
            metrics.node_operations.upsert_time_us < 50,
            "Node upsert took {}μs (>50μs baseline)",
            metrics.node_operations.upsert_time_us
        );
        assert!(
            metrics.query_operations.blast_radius_time_us < 10000,
            "Blast radius took {}μs (>10ms baseline)",
            metrics.query_operations.blast_radius_time_us
        );
        assert!(
            metrics.file_operations.update_time_ms < 10,
            "File update took {}ms (>10ms baseline)",
            metrics.file_operations.update_time_ms
        );
        assert!(
            metrics.memory_usage.total_memory_mb < 15,
            "Memory usage {}MB (>15MB baseline)",
            metrics.memory_usage.total_memory_mb
        );
    ...
    fn test_small_workload_optimal() {
        ...
        assert!(
            metrics.node_operations.upsert_time_us < 50,
            "Node upsert took {}μs (>50μs optimal)",
            metrics.node_operations.upsert_time_us
        );
        assert!(
            metrics.query_operations.blast_radius_time_us < 2000,
            "Blast radius took {}μs (>2ms optimal)",
            metrics.query_operations.blast_radius_time_us
        );
        assert!(
            metrics.file_operations.update_time_ms < 5,
            "File update took {}ms (>5ms optimal)",
            metrics.file_operations.update_time_ms
        );
        assert!(
            metrics.memory_usage.total_memory_mb < 10,
            "Memory usage {}MB (>10MB optimal)",
            metrics.memory_usage.total_memory_mb
        );
    ...
    fn test_performance_regression_detection() {
        ...
        for i in 1..baseline_metrics.len() {
            ...
            println!(
                "📊 Performance scaling from {} to {}: {:.2}x upsert time",
    ...
    fn test_memory_efficiency_validation() {
        ...
        assert!(
            metrics.memory_usage.total_memory_mb < expected_memory_mb + 10,
        ...
        assert!(
            metrics.memory_usage.memory_per_edge_bytes < 100,
        ...
        println!(
            "   📊 Memory per node: {} bytes (target: <500 bytes)",
            metrics.memory_usage.memory_per_node_bytes
        );
        println!(
            "   📊 Memory per edge: {} bytes (target: <100 bytes)",
            metrics.memory_usage.memory_per_edge_bytes
        );
        println!(
            "   📊 Total memory: {}MB for {} nodes (efficiency: {:.1} bytes/node)",
            metrics.memory_usage.total_memory_mb,
            config.node_count,
    ...
    fn test_cross_platform_consistency_comprehensive() {
        ...
        let performance_variance = if metrics.node_operations.upsert_time_us > 0 {
            ((metrics.node_operations.upsert_time_us as f64
                - metrics2.node_operations.upsert_time_us as f64)
                .abs()
                / metrics.node_operations.upsert_time_us as f64)
                * 100.0
        } else {
        ...
        for signature in &test_signatures {
            ...
            assert_eq!(
                hash1, hash2,
                "❌ Hash inconsistency for '{}' - Deterministic hashing broken",
                signature
            );
        }
        ...
        println!(
            "   📊 Performance variance: {:.1}% (target: <200%)",
            performance_variance
        );
        println!(
            "   📊 Deterministic hashing: verified for {} test signatures",
            test_signatures.len()
        );
    }
    ...
    fn test_performance_monitoring_and_regression_detection() {
        ...
        let expected_scaling =
            WorkloadConfig::large().node_count as f64 / WorkloadConfig::small().node_count as f64;

        // Allow for reasonable overhead in memory scaling (5x tolerance for small workloads)
        let tolerance_factor = if small_metrics.memory_usage.total_memory_mb <= 1 {
            10.0
        } else {
            3.0
        };
        assert!(
            memory_scaling_ratio < expected_scaling * tolerance_factor,
            "❌ Memory scaled {:.1}x but expected ~{:.1}x - Memory efficiency degraded",
            memory_scaling_ratio,
            expected_scaling
        );

        // Query performance should remain bounded
        assert!(
            large_metrics.query_operations.blast_radius_time_us < 2000,
        ...
        println!(
            "   📊 Node operation scaling: {:.2}x (small→medium), {:.2}x (medium→large)",
            small_to_medium_ratio, medium_to_large_ratio
        );
        println!(
            "   📊 Memory scaling: {:.2}x actual vs {:.2}x expected",
    ...
    fn test_realistic_data_generation() {
        ...
        assert!(
            isg.edge_count() <= config.edge_count * 2,
    ...
    fn test_realistic_rust_codebase_patterns() {
        ...
        println!(
            "   📊 Blast radius: {} dependencies in {}μs",
            blast_radius.len(),
            blast_radius_time.as_micros()
        );
        println!(
            "   📊 Implementors: {} found in {}μs",
            implementors.len(),
            implementors_time.as_micros()
        );
    }
    ...
    fn test_concurrent_performance_under_load() {
                ...
                for i in 0..100 {
                    ...
                    match i % 4 {
                        ...
                        _ => {
                            // Test caller search
                            if let Some(nodes) =
                                isg_clone.find_by_name("process_0").first().copied()
                            {
        ...
        assert!(
            avg_time < 1000,
            "❌ Average concurrent operation took {}μs (>1ms)",
            avg_time
        );
        assert!(
            max_time < 5000,
            "❌ Worst concurrent operation took {}μs (>5ms)",
            max_time
        );
        ...
        println!(
            "   📊 Performance: {}μs avg, {}μs min, {}μs max",
