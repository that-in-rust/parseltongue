...
impl CrossPlatformTestRunner {
    ...
    pub fn run_tests(&self) -> CrossPlatformTestResults {
        ...
        println!(
            "✅ Cross-platform tests completed in {:.2}s",
            elapsed.as_secs_f64()
        );
    ...
    fn collect_platform_info(&self) -> PlatformInfo {
        PlatformInfo {
            ...
            cargo_version: std::env::var("CARGO_VERSION").unwrap_or_else(|_| "unknown".to_string()),
    ...
    fn test_query_performance(&self, isg: &OptimizedISG) -> QueryPerformanceResults {
        ...
        // Calculate averages
        let blast_radius_avg =
            blast_radius_times.iter().sum::<u64>() / blast_radius_times.len() as u64;
        let implementors_avg =
            implementors_times.iter().sum::<u64>() / implementors_times.len() as u64;
    ...
    fn test_hash_generation_performance(&self) -> HashGenerationResults {
        ...
        for _ in 0..self.config.test_iterations * 10 {
            // More iterations for hash timing
            let start = Instant::now();
        ...
        println!(
            "   📊 Hash generation: {}ns avg, {} hashes/sec, consistent: {}",
    ...
    fn test_graph_operations_performance(&self, isg: &OptimizedISG) -> GraphOperationResults {
        ...
        // Calculate averages
        let node_upsert_avg =
            node_upsert_times.iter().sum::<u64>() / node_upsert_times.len() as u64;
        let lookup_avg = lookup_times.iter().sum::<u64>() / lookup_times.len() as u64;

        println!(
            "   📊 Graph operations - Node upsert: {}μs, Lookup: {}μs",
    ...
    fn test_graph_consistency(&self) -> GraphConsistencyResults {
        ...
        let graphs: Vec<OptimizedISG> = (0..3).map(|_| self.create_test_graph()).collect();

        // Test structure consistency
        let first_node_count = graphs[0].node_count();
        let first_edge_count = graphs[0].edge_count();

        let identical_structure = graphs
            .iter()
            .all(|g| g.node_count() == first_node_count && g.edge_count() == first_edge_count);

        // Test query result consistency
        let main_hash = SigHash::from_signature("fn main()");
        let first_blast_radius = graphs[0].calculate_blast_radius(main_hash).unwrap();

        let deterministic_queries = graphs
            .iter()
            .all(|g| g.calculate_blast_radius(main_hash).unwrap() == first_blast_radius);

        // Test serialization consistency (simplified)
        let serialization_consistent = true; // TODO: Implement actual serialization test

        println!(
            "   📊 Graph consistency - Structure: {}, Queries: {}, Serialization: {}",
    ...
    fn create_test_graph(&self) -> OptimizedISG {
        ...
        let nodes = vec![
            ...
            (
                "struct User",
                parseltongue::NodeKind::Struct,
                "struct User { name: String, age: u32 }",
            ),
            (
                "trait Display",
                parseltongue::NodeKind::Trait,
                "trait Display { fn fmt(&self) -> String; }",
            ),
            (
                "fn create_user",
                parseltongue::NodeKind::Function,
        ...
        let user_hash = SigHash::from_signature("struct User { name: String, age: u32 }");
        let display_hash = SigHash::from_signature("trait Display { fn fmt(&self) -> String; }");
        let create_user_hash =
            SigHash::from_signature("fn create_user(name: String, age: u32) -> User");

        isg.upsert_edge(main_hash, create_user_hash, parseltongue::EdgeKind::Calls)
            .unwrap();
        isg.upsert_edge(create_user_hash, user_hash, parseltongue::EdgeKind::Uses)
            .unwrap();
        isg.upsert_edge(user_hash, display_hash, parseltongue::EdgeKind::Implements)
            .unwrap();
    ...
    fn calculate_test_summary(
        ...
        if graph_results.identical_structure_across_runs {
            passed_tests += 1;
        } else {
            failed_tests += 1;
        }
        if graph_results.deterministic_query_results {
            passed_tests += 1;
        } else {
            failed_tests += 1;
        }
        if graph_results.serialization_consistency {
            passed_tests += 1;
        } else {
            failed_tests += 1;
        }
        if graph_results.node_count_consistency {
            passed_tests += 1;
        } else {
            failed_tests += 1;
        }
        if graph_results.edge_count_consistency {
            passed_tests += 1;
        } else {
            failed_tests += 1;
        }
...
impl CrossPlatformTestResults {
    ...
    pub fn print_results(&self) {
        ...
        println!(
            "   Signatures tested: {}",
            self.hash_consistency_results.signatures_tested
        );
        println!(
            "   Consistent: {}",
            self.hash_consistency_results.consistent_hashes
        );
        println!(
            "   Consistency rate: {:.1}%",
            self.hash_consistency_results.consistency_rate
        );
        ...
        if !self.hash_consistency_results.inconsistent_hashes.is_empty() {
            ...
            for inconsistency in &self.hash_consistency_results.inconsistent_hashes {
                println!(
                    "      '{}': {:?}",
                    inconsistency.signature, inconsistency.hash_values
                );
            }
        }
        ...
        println!(
            "   Find implementors: {}μs",
            self.performance_results
                .query_performance
                .find_implementors_avg_us
        );
        println!(
            "   Hash generation: {}ns",
            self.performance_results
        ...
        println!(
            ...
            self.graph_consistency_results
                .identical_structure_across_runs
        );
        println!(
            "   Query determinism: {}",
            self.graph_consistency_results.deterministic_query_results
        );
        println!(
            "   Serialization consistency: {}",
            self.graph_consistency_results.serialization_consistency
        );
        ...
        if self.test_summary.success_rate >= 100.0 {
            ...
            println!(
                "\n⚠️  Cross-platform tests mostly passed ({:.1}%)",
                self.test_summary.success_rate
            );
        } else {
            println!(
                "\n❌ Cross-platform tests FAILED ({:.1}% success rate)",
                self.test_summary.success_rate
            );
        }
    }
...
mod tests {
    ...
    fn test_cross_platform_runner() {
        ...
        assert!(
            results.test_summary.success_rate >= 95.0,
    ...
    fn test_platform_info_collection() {
        ...
        assert!(
            !platform_info.rust_version.is_empty(),
            "Rust version not detected"
        );
