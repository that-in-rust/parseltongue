...
use parseltongue::{EdgeKind, NodeData, NodeKind, OptimizedISG, SigHash};
...
impl CrossPlatformTestSuite {
    ...
    fn generate_current_platform_data(&self) -> CrossPlatformTestData {
        ...
        let expected_hashes: Vec<u64> = self
            .test_signatures
            .iter()
...
fn test_sighash_determinism() {
    println!(
        "🔍 Testing SigHash determinism on platform: {}",
        std::env::consts::OS
    );
    ...
    for signature in &test_suite.test_signatures {
        ...
        assert_eq!(
            hash1, hash2,
            "❌ SigHash not deterministic for signature: '{}'",
            signature
        );
        ...
        for _ in 0..10 {
            ...
            assert_eq!(
                hash1, hash_n,
                "❌ SigHash inconsistent across iterations for: '{}'",
                signature
            );
        }
    }
    ...
    println!(
        "✅ SigHash determinism verified for {} signatures",
        test_suite.test_signatures.len()
    );
}
...
fn test_cross_platform_hash_consistency() {
    println!(
        "🌍 Testing cross-platform hash consistency on: {}",
        std::env::consts::OS
    );
    ...
    let expected_hashes = vec![
        ...
        (
            ...
            0x7b2a_3f8d_9e1c_4567_u64,
        ),
        (
            "trait Display { fn fmt(&self) -> String; }",
            0x9d4e_6a2b_5c8f_1234_u64,
        ),
    ];
    ...
    for (signature, _expected_hash) in &expected_hashes {
        ...
        assert_eq!(
            computed_hash, hash2,
            "❌ Hash inconsistency within platform for: '{}'",
            signature
        );
...
fn test_graph_structure_determinism() {
    println!(
        "🏗️  Testing graph structure determinism on: {}",
        std::env::consts::OS
    );
    ...
    assert_eq!(
        structure1.node_count, structure2.node_count,
        "❌ Node count mismatch between identical graphs"
    );

    assert_eq!(
        structure1.edge_count, structure2.edge_count,
    ...
    println!(
        "   📊 Nodes: {}, Edges: {}",
        structure1.node_count, structure1.edge_count
    );
}
...
fn test_serialization_consistency() {
    println!(
        "💾 Testing serialization consistency on: {}",
        std::env::consts::OS
    );
    ...
    let json = serde_json::to_string(&test_data).expect("Failed to serialize test data");

    let deserialized: CrossPlatformTestData =
        serde_json::from_str(&json).expect("Failed to deserialize test data");
    ...
    let json2 =
        serde_json::to_string(&test_data).expect("Failed to serialize test data (second time)");

    // Note: JSON serialization order may vary, so we compare deserialized objects
    let deserialized2: CrossPlatformTestData =
        serde_json::from_str(&json2).expect("Failed to deserialize test data (second time)");

    assert_eq!(
        deserialized, deserialized2,
...
fn test_platform_isolation() {
    println!(
        "🔒 Testing platform behavior isolation on: {}",
        std::env::consts::OS
    );
    ...
    for (signature, _file_path) in &signatures_with_paths {
        ...
        assert_eq!(
            hash, base_hash,
            "❌ File path affected signature hash for: '{}'",
            signature
        );
    }
...
fn test_comprehensive_cross_platform_integration() {
    ...
    assert!(
        !platform_data.test_signatures.is_empty(),
        "❌ No test signatures generated"
    );

    assert_eq!(
        platform_data.test_signatures.len(),
        platform_data.expected_hashes.len(),
        "❌ Signature count mismatch with hash count"
    );

    assert!(
        platform_data.graph_structure.node_count > 0,
        "❌ No nodes in test graph"
    );

    assert!(
        platform_data.graph_structure.edge_count > 0,
    ...
    assert_eq!(
        platform_data.graph_structure.node_count, recreated_structure.node_count,
        "❌ Graph recreation failed - node count mismatch"
    );

    assert_eq!(
        platform_data.graph_structure.edge_count, recreated_structure.edge_count,
        "❌ Graph recreation failed - edge count mismatch"
    );

    // Test query consistency
    let main_hash = SigHash::from_signature("fn main");
    let blast_radius = isg
        .calculate_blast_radius(main_hash)
    ...
    println!(
        "   📊 Graph nodes: {}",
        platform_data.graph_structure.node_count
    );
    println!(
        "   📊 Graph edges: {}",
        platform_data.graph_structure.edge_count
    );
...
fn test_cross_platform_performance_consistency() {
    ...
    for _ in 0..10 {
        ...
        let _result = isg.calculate_blast_radius(main_hash).expect("Query failed");
    ...
    assert!(
        variance_percent < 500.0,
        "❌ Performance variance too high: {:.1}% (max: 500%)",
        variance_percent
    );

    // Ensure performance meets targets
    assert!(
        avg_time < 1000, // 1ms target
        "❌ Average query time {}μs exceeds 1ms target",
        avg_time
    );
