...
use parseltongue::discovery::{AnalysisSession, WorkspaceError, WorkspaceManager};
...
async fn test_workspace_complex_workflow_data() {
    ...
    manager
        .store_workflow_result("onboard", &onboard_result)
        .await
        .unwrap();
    ...
    manager
        .store_workflow_result("feature-start", &feature_result)
        .await
        .unwrap();

    // Retrieve and verify both results
    let retrieved_onboard: Option<serde_json::Value> =
        manager.get_cached_result("onboard").await.unwrap();

    let retrieved_feature: Option<serde_json::Value> =
        manager.get_cached_result("feature-start").await.unwrap();
    ...
    assert_eq!(
        feature_data["change_checklist"].as_array().unwrap().len(),
        3
    );
    assert_eq!(
        feature_data["test_recommendations"]
            .as_array()
            .unwrap()
            .len(),
        3
...
async fn test_workspace_automatic_latest_linking() {
    ...
    let _session2 = manager.get_or_create_session(true).await.unwrap();
    tokio::time::sleep(tokio::time::Duration::from_millis(10)).await;
