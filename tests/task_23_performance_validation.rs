...
async fn test_discovery_performance_contracts() {
    ...
    let _ingestion_time = validator
        .ingest_codebase(&iggy_code)
        .await
    ...
    assert!(
        total_discovery_time < Duration::from_secs(15),
        "Discovery took {:?}, expected <15s",
        total_discovery_time
    );

    println!(
        "    ✅ Iggy-scale: {} entities discovered in {:.2}s",
        entities.len(),
        total_discovery_time.as_secs_f64()
    );
    ...
    assert!(
        axum_discovery_time < Duration::from_secs(8),
        "Axum discovery took {:?}, expected <8s",
        axum_discovery_time
    );

    println!(
        "    ✅ Axum-scale: {} entities discovered in {:.2}s",
        axum_entities.len(),
        axum_discovery_time.as_secs_f64()
    );
...
async fn test_query_performance_contracts() {
    ...
    assert!(
        avg_discovery_time < Duration::from_millis(150),
        "Average discovery query took {:?}, expected <150ms",
        avg_discovery_time
    );
    assert!(
        max_discovery_time < Duration::from_millis(200),
        "Max discovery query took {:?}, expected <200ms",
        max_discovery_time
    );

    println!(
        "    ✅ Discovery queries: avg {:.2}ms, max {:.2}ms",
    ...
    if !existing_times.is_empty() {
        ...
        assert!(
            avg_existing_time < Duration::from_millis(100),
            "Average existing query took {:?}, expected <100ms",
            avg_existing_time
        );

        println!(
            "    ✅ Existing ISG queries: avg {:.2}ms, max {:.2}ms",
...
async fn test_jtbd_workflow_timing_requirements() {
    ...
    assert!(
        onboard_time < Duration::from_secs(10 * 60),
        "Onboard workflow took {:?}, expected <10 minutes",
        onboard_time
    );

    println!(
        "    ✅ Onboard: {:.2}s ({} entities, {} entry points)",
    ...
    println!(
        "    ✅ Feature-start: {:.2}s ({} direct impacts)",
    ...
    assert!(
        debug_time < Duration::from_secs(3 * 60),
        "Debug workflow took {:?}, expected <3 minutes",
        debug_time
    );

    println!(
        "    ✅ Debug: {:.2}s ({} caller traces, {} usage sites)",
    ...
    assert!(
        refactor_time < Duration::from_secs(4 * 60),
        "Refactor-check workflow took {:?}, expected <4 minutes",
        refactor_time
    );

    println!(
        "    ✅ Refactor-check: {:.2}s ({} risk factors, {} checklist items)",
    ...
    println!(
        ...
        total_workflow_time.as_secs_f64()
    );
...
async fn test_memory_usage_monitoring() {
    ...
    let ingestion_increase =
        ((post_ingestion_memory as f64 - baseline_memory as f64) / baseline_memory as f64) * 100.0;
    let operations_increase =
        ((post_operations_memory as f64 - baseline_memory as f64) / baseline_memory as f64) * 100.0;

    // Validate memory usage contract (<20% increase, using <50% for test)
    println!("  📈 Memory usage analysis:");
    println!("    Baseline: {} units", baseline_memory);
    println!(
        "    Post-ingestion: {} units ({:.1}% increase)",
        post_ingestion_memory, ingestion_increase
    );
    println!(
        "    Post-operations: {} units ({:.1}% increase)",
        post_operations_memory, operations_increase
    );
    ...
    if operations_increase > 50.0 {
        test_validator.record_violation(format!(
            "Memory usage increase violation: {:.1}% > 50%",
            operations_increase
        ));
    }

    assert!(
        operations_increase < 60.0,
        "Memory usage increased by {:.1}%, expected <60%",
        operations_increase
    );
    ...
    let stability_change = ((stability_end_memory as f64 - stability_start_memory as f64)
        / stability_start_memory as f64)
        * 100.0;

    println!(
        "    Memory stability: {:.1}% change over 100 operations",
        stability_change
    );

    // Memory should remain stable during repeated operations
    assert!(
        stability_change.abs() < 10.0,
        "Memory usage changed by {:.1}% during stability test",
        stability_change
    );
...
async fn test_comprehensive_system_integration() {
    ...
    let onboard_result = orchestrator
        .onboard("integration_test")
        .await
        .expect("Onboard workflow should succeed");

    // Step 8: Store results in workspace
    workspace_manager
        .store_workflow_result("integration_test", &onboard_result)
        .await
    ...
    let retrieved_result: serde_json::Value = workspace_manager
        .get_cached_result("integration_test")
        .await
    ...
    assert!(
        integration_time < Duration::from_secs(30),
        "Complete integration took {:?}, expected <30s",
        integration_time
    );
...
async fn test_generate_performance_validation_report() {
    ...
    println!(
        "• Discovery <30s for realistic codebases: {}",
        if discovery_time < Duration::from_secs(30) {
            "✅ PASS"
        } else {
            "❌ FAIL"
        }
    ...
    println!(
        "• Queries <100ms for interactive use: {}",
    ...
    println!(
        "• JTBD workflows within time limits: {}",
        if workflow_time < Duration::from_secs(15 * 60) {
            "✅ PASS"
        } else {
            "❌ FAIL"
        }
    ...
    assert!(
        discovery_time < Duration::from_secs(15),
        "Discovery performance acceptable"
    );
    assert!(
    ...
    assert!(
        workflow_time < Duration::from_secs(10 * 60),
