...
use parseltongue::discovery::{types::EntityType, DiscoveryEngine, SimpleDiscoveryEngine};
use parseltongue::isg::{NodeData, NodeKind, OptimizedISG, SigHash};
...
mod discovery_to_analysis_workflow {
    ...
    async fn test_complete_discovery_to_analysis_workflow() {
        ...
        let functions_result = engine
            .list_all_entities(Some(EntityType::Function), 1000)
            .await;
        let structs_result = engine
            .list_all_entities(Some(EntityType::Struct), 1000)
            .await;
        ...
        if all_entities_result.is_ok() {
            ...
            assert!(
                !entities.is_empty(),
                "Should find entities in realistic codebase"
            );
...
mod realistic_codebase_stress_tests {
    ...
    async fn test_iggy_scale_stress_test() {
        ...
        assert!(
            discovery_time < Duration::from_secs(10),
            "Discovery initialization took {:?}, expected <10s",
            discovery_time
        );
        ...
        for i in 0..total_queries {
            ...
            let result = match i % 4 {
                ...
                1 => engine
                    ...
                    .map(|_| ()),
                2 => engine
                    .entities_in_file(&format!("src/file_{}.rs", i % file_count))
                    .await
                    .map(|_| ()),
                _ => engine
                    .where_defined(&format!(
                        "function_{}",
                        i % (file_count * entities_per_file)
                    ))
                    .await
                    .map(|_| ()),
            };
        ...
        assert!(
            health_result.is_ok(),
            "System health check failed under load"
        );

        println!("✅ Iggy-scale stress test completed:");
        println!(
            "   Files: {}, Entities per file: {}",
            file_count, entities_per_file
        );
        println!("   Discovery time: {:.2}s", discovery_time.as_secs_f64());
        ...
        println!(
            "   Max query time: {:.2}ms",
            max_query_time.as_secs_f64() * 1000.0
        );
    }
    ...
    async fn test_axum_scale_stress_test() {
        ...
        assert!(
            discovery_time < Duration::from_secs(5),
            "Discovery took {:?}, expected <5s for Axum scale",
            discovery_time
        );
        ...
        for entity_type in [EntityType::Function, EntityType::Struct, EntityType::Trait] {
            total_operations += 1;
            if engine
                .list_all_entities(Some(entity_type), 200)
                .await
                .is_ok()
            {
                successful_operations += 1;
        ...
        assert!(
            success_rate >= 0.9,
            "Success rate {:.1}% below 90% for Axum scale",
            success_rate * 100.0
        );
    ...
    async fn test_concurrent_query_stress() {
        ...
        let _baseline_result = engine
            .list_all_entities(None, 600)
            .await
            ...
            let handle = tokio::spawn(async move {
                match i % 4 {
                    ...
                    1 => engine_clone
                        ...
                        .map(|_| ()),
                    2 => engine_clone
                        .entities_in_file(&format!("src/file_{}.rs", i % 50))
                        .await
                        .map(|_| ()),
                    _ => engine_clone
                        .where_defined(&format!("function_{}", i))
                        .await
                        .map(|_| ()),
                }
            });
        ...
        assert!(
            success_rate >= 0.9,
            "Concurrent success rate {:.1}% below 90%",
            success_rate * 100.0
        );
        ...
        assert!(
            ...
            "Performance degradation {:.1}x exceeds 2x limit",
            degradation_ratio
        );
...
mod success_metrics_validation {
    ...
    async fn test_discovery_time_performance_contract() {
        ...
        for (file_count, entities_per_file, max_time) in test_cases {
            ...
            assert!(
                ...
                "ISG creation took {:?}, expected <2s",
                init_time
            );
            assert!(
                engine_time < Duration::from_millis(100),
                "Engine creation took {:?}, expected <100ms",
                engine_time
            );
            assert!(
                query_time < Duration::from_millis(500),
                "First query took {:?}, expected <500ms",
                query_time
            );
            assert!(
                total_time < max_time,
                "Total discovery time {:?} exceeded limit {:?}",
                total_time,
                max_time
            );
            ...
            println!(
                "    ✅ Init: {:.2}ms, Engine: {:.2}ms, Query: {:.2}ms, Total: {:.2}ms",
    ...
    async fn test_success_rate_contract() {
        ...
        assert!(
            discovery_rate >= 0.95,
            "Entity discovery success rate {:.1}% below 95%",
        ...
        for i in 0..query_tests {
            let result = match i % 4 {
                ...
                1 => engine
                    .entities_in_file(&format!("src/file_{}.rs", i % 80))
                    .await
                    .map(|_| ()),
                2 => engine
                    .where_defined(&format!("function_{}", i % 640))
                    .await
                    .map(|_| ()),
        ...
        assert!(
            query_rate >= 0.90,
            "Query execution success rate {:.1}% below 90%",
            query_rate * 100.0
        );
        ...
        assert!(
            file_nav_rate >= 0.95,
            "File navigation success rate {:.1}% below 95%",
            file_nav_rate * 100.0
        );
        ...
        assert!(
            edge_case_rate >= 0.90,
            "Edge case handling success rate {:.1}% below 90%",
    ...
    async fn test_query_performance_contract() {
        ...
        let all_entities = engine
            .list_all_entities(None, 1500)
            .await
            .expect("Entity listing should succeed");
        let list_time = start.elapsed();

        assert!(
            list_time < Duration::from_millis(100),
            "Entity listing took {:?}, expected <100ms",
            list_time
        );
        ...
        let functions = engine
            .list_all_entities(Some(EntityType::Function), 500)
            .await
        ...
        assert!(
            ...
            "Filtered listing took {:?}, expected <100ms",
            filter_time
        );
        assert!(functions
            .iter()
        ...
        let start = std::time::Instant::now();
        let file_entities = engine
            .entities_in_file("src/file_0.rs")
            .await
            .expect("File query should succeed");
        let file_time = start.elapsed();

        assert!(
            file_time < Duration::from_millis(50),
            "File query took {:?}, expected <50ms",
            file_time
        );
        ...
        let start = std::time::Instant::now();
        let location = engine
            .where_defined("function_0")
            .await
            .expect("Location lookup should succeed");
        let lookup_time = start.elapsed();

        assert!(
            lookup_time < Duration::from_millis(25),
            "Location lookup took {:?}, expected <25ms",
            lookup_time
        );
        ...
        assert!(
            avg_rapid_time < Duration::from_millis(25),
            "Average rapid query took {:?}, expected <25ms",
            avg_rapid_time
        );
...
mod end_to_end_workflow_validation {
    ...
    async fn test_sarah_discovery_workflow() {
        ...
        assert!(
            discovery_time < Duration::from_secs(5),
            "Entity discovery took {:?}, expected <5s",
            discovery_time
        );
        assert!(
            all_entities.len() > 400,
            "Should find substantial number of entities"
        );

        // Step 3: Browse entities by type
        let functions = engine
            .list_all_entities(Some(EntityType::Function), 200)
            .await
            .expect("Should find functions");
        let structs = engine
            .list_all_entities(Some(EntityType::Struct), 200)
            .await
            .expect("Should find structs");
        let traits = engine
            .list_all_entities(Some(EntityType::Trait), 200)
            .await
        ...
        let location = engine
            .where_defined(&sample_entity.name)
            .await
        ...
        let file_entities = engine
            .entities_in_file(&loc.file_path)
            .await
            .expect("Should find entities in file");
        assert!(
            !file_entities.is_empty(),
            "Should find entities in the file"
        );
        ...
        assert!(
            workflow_time < Duration::from_secs(120),
            "Complete workflow took {:?}, expected <2 minutes",
            workflow_time
        );
