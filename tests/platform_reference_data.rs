...
fn create_reference_graph() -> ReferenceGraph {
    ...
    for (source_sig, target_sig, edge_kind) in &edge_definitions {
        ...
        isg.upsert_edge(source_hash, target_hash, *edge_kind)
            .unwrap();
...
pub fn validate_against_reference(
    reference: &PlatformReferenceData,
    ...
    for (signature, expected_hash) in &reference.reference_hashes {
        ...
        if computed_hash == *expected_hash {
            ...
            report
                .failed_tests
                .push(format!("Hash mismatch for '{}'", signature));
    ...
    for test_case in &reference.test_cases {
        ...
        if computed_hash == test_case.expected_hash {
            ...
            report
                .failed_tests
                .push(format!("Test case '{}' failed", test_case.name));
            report.hash_mismatches.push(HashMismatch {
...
mod tests {
    ...
    fn test_reference_data_generation() {
        ...
        assert!(
            !reference.reference_hashes.is_empty(),
            "No reference hashes generated"
        );
        assert!(
            !reference.reference_graph.nodes.is_empty(),
            "No reference nodes generated"
        );
        assert!(
            !reference.reference_graph.edges.is_empty(),
            "No reference edges generated"
        );
        ...
        println!(
            "   📊 Reference hashes: {}",
            reference.reference_hashes.len()
        );
        println!(
            "   📊 Reference nodes: {}",
            reference.reference_graph.nodes.len()
        );
        println!(
            "   📊 Reference edges: {}",
            reference.reference_graph.edges.len()
        );
    ...
    fn test_self_validation() {
        let reference = generate_reference_data();
        let report = validate_against_reference(&reference).expect("Validation failed");
        ...
        assert_eq!(
            report.success_rate(),
            100.0,
            "Self-validation should be 100% successful"
        );
    ...
    fn test_hash_consistency() {
        ...
        assert_eq!(
            reference.reference_hashes.len(),
            reference2.reference_hashes.len(),
            "Reference hash count changed between generations"
        );

        for (signature, hash1) in &reference.reference_hashes {
            let hash2 = reference2
                .reference_hashes
                .get(signature)
                .unwrap_or_else(|| {
                    panic!("Signature '{}' missing in second generation", signature)
                });

            assert_eq!(
                hash1, hash2,
                "Hash inconsistency for '{}' between generations",
                signature
            );
        }
