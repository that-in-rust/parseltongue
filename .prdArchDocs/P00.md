...
**Purpose**: Single comprehensive Mermaid diagram representing the complete Parseltongue workflow from P01 minimal PRD with exquisite detail.

**Audience**: System architects, developers, and stakeholders who need to understand the complete end-to-end workflow.

**Philosophy**: One detailed visual truth source combining all PRD layers (P01-P06) with phase-based organization.

---

## Complete System Workflow - Ultra-Detailed Visualization
...
flowchart TB
    ...
    %% ============================================================================
    %% PHASE 2: BUG ANALYSIS & MICRO-PRD CREATION
    %% ============================================================================
    subgraph Phase2Init ["🐛 PHASE 2A: BUG ANALYSIS & MICRO-PRD"]
    ...
    %% ============================================================================
    %% PHASE 4: VALIDATION & CODE WRITING
    %% ============================================================================
    subgraph Phase4 ["✅ PHASE 4: VALIDATION & CODE WRITING"]

        B02Success --> StepC["<b>STEP C:</b> Syntax Validation (Tool 4)"]

        subgraph Tool4Validation ["Tool 4: rust-preflight-code-simulator"]
            StepC --> Tool4Cmd["<b>COMMAND:</b><br/>rust-preflight-code-simulator<br/>validation_output.json<br/>--validation-type all"]

            Tool4Cmd --> Tool4Scope["<b>ULTRA-MINIMALIST SCOPE:</b><br/>• Tree-sitter syntax validation ONLY<br/>• Check entities with future_code<br/>• <20ms for typical changes (50 entities)<br/><b>DOES NOT VALIDATE:</b><br/>× Types (cargo handles)<br/>× Imports (cargo handles)<br/>× Lifetimes (cargo handles)<br/>× Logic (tests handle)"]

            Tool4Scope --> Tool4Check["Validate syntax:<br/>• Missing brackets?<br/>• Malformed expressions?<br/>• Keyword typos?"]
...
| **Phase 2A** | Bug analysis | Tool 3 (read) | Micro-PRD iterations (2x) | Yes (2 iterations) |
| **Phase 3** | Temporal simulation | Tools 2 & 3 (edit + read) | Confidence ≥80% | **YES** (unlimited A→B cycles) |
| **Phase 4** | Validation & writing | Tools 4 & 5, cargo | Build/test validation | Loops back to Phase 3 on failure |
| **Phase 5** | State reset | Tool 6, git | User satisfaction | Loops back to Phase 3 if unsatisfied |

### Critical Loops & Iteration Points

1. **Micro-PRD Refinement** (Phase 2A): Fixed 2 iterations to clarify requirements
2. **Temporal Simulation Cycle** (Phase 3): Unlimited A01→A02→B01→B02 loops until LLM confidence ≥80%
3. **Validation Failures** (Phase 4):
   - Syntax errors → Back to A01 (interface-level changes)
   - Build errors → Back to A01 (type/import issues)
   - Test failures → Back to B01 (code-level fixes)
4. **User Dissatisfaction** (Phase 5): Back to A01 for major rework
...
├─────────────────────────────────────────────────────────────┤
│ DEFAULT STATE (Phase 2A initial):                          │
...
├─────────────────────────────────────────────────────────────┤
│ CHANGING ENTITIES ONLY (Phase 3, Step B01):                │
