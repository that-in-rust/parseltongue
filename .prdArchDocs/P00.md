...
---
flowchart TD
    Start([User invokes agent in Git repo]) --> Setup["Setup & Binary Installation<br/>Confirm repository location"]

    Setup --> Phase1["Phase 1: ISG Creation<br/>(Interface Signature Graph)<br/>folder-to-cozoDB-streamer<br/>Parse codebase → CozoDB<br/><30s for 50k LOC"]

    Phase1 --> Phase2["Phase 2: MicroPRD Refinement<br/>LLM-cozoDB-to-context-writer<br/>Extract signatures only<br/>(NO current_code, NO future_code)<br/>User refines microPRD iteratively<br/>(2 iterations, ~37.5k tokens)"]

    Phase2 --> Phase3["Phase 3: Temporal Simulation<br/>LLM-to-cozoDB-writer (write changes)<br/>LLM-cozoDB-to-context-writer (read context)<br/>Iterative loop: A01→A02→B01→B02<br/>Until confidence ≥80%"]

    Phase3 --> ConfidenceGate{Confidence<br/>≥ 80%?}
    ConfidenceGate --> |"No<br/>Refine"| Phase3
    ConfidenceGate --> |"Yes<br/>Proceed"| Phase4

    Phase4["Phase 4: Pre-Flight & File Writing<br/>rust-preflight-code-simulator<br/>(syntax check future_code, <20ms)<br/>LLM-cozodb-to-diff-writer<br/>(generate CodeDiff.json)<br/>LLM applies → cargo build + test"]
    ...
    Phase5["Phase 5: State Reset<br/>cozoDB-make-future-code-current<br/>Delete table + re-index<br/>Git commit with changes"]
...
**Commands by Phase:**
- **Phase 1**: `folder-to-cozoDB-streamer` (ISG creation)
- **Phase 2**: `LLM-cozoDB-to-context-writer` (extract signatures only)
- **Phase 3**: `LLM-to-cozoDB-writer` + `LLM-cozoDB-to-context-writer` (iterative read-edit cycle)
- **Phase 4**: `rust-preflight-code-simulator` (pre-flight syntax), `LLM-cozodb-to-diff-writer` (diff generation), cargo build/test
- **Phase 5**: `cozoDB-make-future-code-current` (state reset), git
...
| **Phase 1** | ISG Creation | `folder-to-cozoDB-streamer` | Rust detection (LSP) | No (one-time per codebase) |
| **Phase 2** | MicroPRD refinement | `LLM-cozoDB-to-context-writer` | MicroPRD iterations (2x) | Yes (2 iterations) |
| **Phase 3** | Temporal simulation | `LLM-to-cozoDB-writer`, `LLM-cozoDB-to-context-writer` | Confidence ≥80% | **YES** (unlimited A→B cycles) |
| **Phase 4** | Pre-flight & file writing | `rust-preflight-code-simulator`, `LLM-cozodb-to-diff-writer`, cargo | Syntax/build/test validation | Loops back to Phase 3 on failure |
| **Phase 5** | State reset | `cozoDB-make-future-code-current`, git | User satisfaction | Loops back to Phase 3 if unsatisfied |
...
- `folder-to-cozodb-streamer` - Indexes codebase using tree-sitter, creates ISGL1 keys in CozoDB
- `llm-to-cozodb-writer` - Accepts LLM-generated queries to update CozoDB with temporal versioning
- `llm-cozodb-to-context-writer` - Extracts optimized context JSON from CozoDB for LLM reasoning
- `rust-preflight-code-simulator` - Validates future_code syntax using tree-sitter (<20ms)
- `llm-cozodb-to-diff-writer` - Generates CodeDiff.json from CozoDB for LLM to apply changes
- `cozodb-make-future-code-current` - Resets temporal state by deleting CodeGraph table and re-indexing
...
- `.claude/commands/` - Optional slash commands for power users (/index, /validate, /reset)
...
@parseltongue-reasoning-orchestrator "Fix panic in GitHub #1234"

# Advanced workflow (5% of users)
/index              # Run Tool 1 manually
/validate           # Run Tool 4 manually
/reset              # Run Tool 6 manually
```

**Architecture Details**: See `.domainDocs/D11-hook-orchestrated-agent-architecture.md` for comprehensive research and Minto Pyramid structured decision rationale.
