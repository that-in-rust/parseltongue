...
### Installation Flow (curl | bash Pattern)

```mermaid
flowchart TD
    Start([User in Git repository]) --> Curl["curl -sSL github.com/.../install.sh | bash"]

    Curl --> Detect["Detect OS/Architecture<br/>(macOS arm64, Linux x64)"]

    Detect --> DownloadBinary["Download single binary<br/>from GitHub Releases<br/>→ ~/.parseltongue/bin/parseltongue"]

    DownloadBinary --> AddPath["Add to PATH<br/>(~/.zshrc)"]

    AddPath --> CreateDirs["Create .claude/ structure<br/>(agents/, hooks/)"]

    CreateDirs --> DownloadAgent["Download agent.md<br/>→ .claude/agents/"]

    DownloadAgent --> DownloadHooks["Download hooks (2 files)<br/>→ .claude/hooks/"]

    DownloadHooks --> CreateSettings["Create settings.local.json<br/>(permissions + hooks)"]

    CreateSettings --> InitDB["mkdir .parseltongue/<br/>+ empty parseltongue.db"]

    InitDB --> Complete["✅ Installation Complete"]

    Complete --> Usage["@parseltongue-reasoning-orchestrator<br/>'Fix panic in #1234'"]

    style Curl fill:#bfdbfe,stroke:#1e40af
    style DownloadBinary fill:#bbf7d0,stroke:#15803d
    style Complete fill:#bbf7d0,stroke:#15803d
    style Usage fill:#fef3c7,stroke:#ca8a04
```
