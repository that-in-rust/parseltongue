...
if ./${BINARY_NAME} --version | grep -q "${BINARY_VERSION}"; then
    ...
    echo "   After restart: Agent persists forever - no more restarts needed."
    echo ""
    echo "Next steps:"
    echo "  1. EXIT and RESTART Claude Code (one-time, loads the agent)"
    echo "  2. Run: ./${BINARY_NAME} --help"
    echo "  3. Read: cat .claude/.parseltongue/parseltongue-README.md"
    echo "  4. Agent: @parseltongue-ultrathink-isg-explorer (now active!)"
    echo ""
    echo "Start indexing:"
